 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Bosun_Sniffs_Commenting_DocCommentSniff implements PHP_CodeSniffer_Sniff
{
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $pos => $tag) {
            if ($pos > 0) {
                $prev = $phpcsFile->findPrevious(
                        T_DOC_COMMENT_STRING, ($tag - 1), $tokens[$commentStart]['comment_tags'][($pos - 1)]
                );
            ...
            if ($tokens[$tag]['content'] === '@param') {
                ...
                if ($fix === true) {
                    $phpcsFile->fixer->beginChangeset();
                    $indent = str_repeat(' ', $tokens[$stackPtr]['column']);
                    $phpcsFile->fixer->addContent($prev, $phpcsFile->eolChar . $indent . '*');
                    $phpcsFile->fixer->endChangeset();
                }
            }//end if
